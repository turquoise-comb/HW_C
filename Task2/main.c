...
char letterline[] = "-----";// displays unguessed letters of the word
...
while(mistakes_remained >= 0){// iterates as long as player has at least 0 mistakes left

	printf("\nThe word to be guessed: %s\n", letterline);
	...
	printf("\nguessed letter = %c\n", guessed_letter);
	///////////////////checking for correct input missing
	...
	if(guessed_correctly == 0){// checks if the user did not guess correctly and the value of the variable is not 1
			printf("\n%c is not a part of the word", guessed_letter);
			mistakes_remained--;// number of mistakes decreases;
		}else printf("\n%c is a part of the word", guessed_letter);// in case the guess is correct prints message
	...
	//strcmp method from string.h library that compares 2 strings is used, strcmp returns 0 if the strings are	
	...
	switch(mistakes_remained){
		...
		default:
			printf("Ooops!Something went terribly wrong.");	
 
    
	}//end of switch
	if(mistakes_remained < 0) break;
